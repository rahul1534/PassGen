...
make test     # go test ./...
make test-ui  # run Playwright UI tests (builds and serves the app)
make format   # format all tracked Go files
make build    # produce dist/
make production-check # verify dist/ contains no development tooling
make dev      # build + serve dist/ with Go (PORT, default 8080)
make clean    # remove dist/
make privacy  # privacy regression checks
```

The UI tests require Node.js, `npm install`, and `npx playwright install chromium`.
